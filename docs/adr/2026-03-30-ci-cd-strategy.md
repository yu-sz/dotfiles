...
## Addendum (2026-04-12): Linux 実ビルド CI の追加

Status: **Accepted** — 方針変更なし、CI カバレッジの拡充

### 経緯

Linux support（standalone home-manager）の実装に伴い、Linux HM 設定の実ビルド検証が必要になった。

### 変更した決定

| 項目     | 元の決定                              | 変更後                                                                             |
| -------- | ------------------------------------- | ---------------------------------------------------------------------------------- |
| Linux CI | lint + dry-run のみ（`nix-lint.yml`） | `nix-build-linux.yml` を追加し、PR の `flake.lock` 変更時 + 手動トリガーで実ビルド |
