...
`push` イベントのみで実行する。ブランチ制限なし（main でも feature ブランチでも発火）。どのブランチへの push でも1回だけ実行され、`pull_request` との重複が発生しない。ローカルの pre-commit hooks が第一防衛線、CI はセーフティネット。Nix 関連ファイル（`**.nix`, `flake.lock`, `Justfile`, `scripts/**`, CI ワークフロー）の変更時のみ paths フィルタで実行を制限する。
...
> **Note**: ローカルインストール（`scripts/setup/prepare_env.sh`）では `NixOS/nix-installer`（Rust 製バイナリインストーラ、`artifacts.nixos.org/nix-installer` 経由）を使用している。CI の `cachix/install-nix-action` とはインストール方法が異なるが、どちらも upstream Nix をインストールする点は同一。ローカルは対話的セットアップ（daemon 構成等）に適した nix-installer、CI は GitHub Actions に特化した cachix 版、という使い分け。
