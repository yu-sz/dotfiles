...
`system.activationScripts.postActivation` で `activateSettings -u` を実行し、`drs` 後にログアウトなしで設定を即時反映する。

> **実装時の修正**: `postUserActivation` は nix-darwin で削除済みのため `postActivation` を使用。
