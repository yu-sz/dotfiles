...
## Addendum (2026-04-12): Linux 展開の実施

Status: **Accepted** — 方針変更なし、計画していた Linux 展開を実施

### 経緯

standalone home-manager による Linux サポートを実装した。共通パッケージ定義はそのまま再利用できており、当初の設計判断が妥当だったことを確認。

### 補足

- `flake.nix` に `mkHomeConfig` ヘルパーと `homeConfigurations` を追加
- `nix/home/linux.nix` で Linux 固有設定（`targets.genericLinux`、GUI アプリ、フォント）を管理
- 詳細は [Linux support ADR](2026-04-11-linux-support-strategy.md) を参照
