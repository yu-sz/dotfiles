...
`.zshrc` の brew・sheldon は **write-through キャッシュ**パターンを使用（ソースの mtime > キャッシュなら再生成）。新しいツールを追加する場合も同じパターンに従うこと。

例外: mise は Nix store 由来で mtime が 0 のため、キャッシュ先頭に記録した実パス（ストアハッシュ）の一致で再生成を判定する。

Sheldon 固有: `sheldon::load` が設定/スクリプトの mtime 変更を検出し自動再構築。ファイル**削除**は mtime では検知できないため手動で `sheldon lock` を実行する。プラグイン更新は `sheldon::update` を使用。
...
`config/zsh/plugins/<name>/` に `plugin.zsh`・`bin/`・`completions/` を置き、plugins.toml の `local` ソースで読み込む（例: workspace。PATH の即時反映が必要なため eager で source）。
